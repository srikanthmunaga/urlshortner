...
class UrlControllerIntegrationTest {
    ...
    void createUrl_conflictsOnDuplicateCustomAlias() throws Exception {
        Map<String, Object> first = Map.of("longUrl", "https://example.com/a", "customAlias", "uniquex1");
        mockMvc.perform(post("/api/urls").contentType("application/json")
                        .content(objectMapper.writeValueAsString(first)))
                .andExpect(status().isCreated());

        Map<String, Object> second = Map.of("longUrl", "https://example.com/b", "customAlias", "uniquex1");
        mockMvc.perform(post("/api/urls").contentType("application/json")
