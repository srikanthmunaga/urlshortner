...
public class UrlServiceImpl implements UrlService {
    ...
    public UrlStatsResponse getStats(String shortCode) {
        ...
        long since24h = clickEventRepository.countSince(shortCode, Instant.now().minus(24, ChronoUnit.HOURS));
        List<ClickEvent> recent = clickEventRepository.findTop50ByShortCodeOrderByClickedAtDesc(shortCode);

        List<UrlStatsResponse.RecentClick> recentClicks = recent.stream()
                .map(c -> new UrlStatsResponse.RecentClick(c.getClickedAt(), c.getReferrer(), c.getUserAgent()))
                .toList();
