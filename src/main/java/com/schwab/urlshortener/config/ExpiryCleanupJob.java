...
/**
 * Background reliability job: periodically sweeps expired-but-still-active rows
 * and marks them inactive so reads consistently reflect expiry without relying
 * solely on the on-read isExpired() check (defense in depth + keeps cache/DB
 * state converging even for codes nobody looks up again).
 */
@Component
